...
// isOrphaned tries to detect if there are no mutations left to be done for the job which
// will make it a candidate for garbage collection. Jobs can be left in such inconsistent state
// if they fail before being removed from the jobs table.
func (r *Registry) isOrphaned(ctx context.Context, payload *jobspb.Payload) (bool, error) {
	for _, id := range payload.DescriptorIDs {
		...
		if err == nil {
			...
			_, err := sqlbase.GetDatabaseDescFromID(ctx, txn, id)
			if err != nil {
				return false, errors.Errorf(
					"Descriptor %d is neither a table or db descriptor", id)
			}
			return false, nil
		}
		txn.Commit(ctx)
...
func (r *Registry) cleanupOldJobs(ctx context.Context, olderThan time.Time) error {
	...
	if len(toDelete.Array) > 0 {
		...
		log.Infof(ctx, "cleaning up %v expired job records", toDelete)
